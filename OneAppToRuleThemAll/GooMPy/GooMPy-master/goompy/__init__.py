...
try:
    ...
    _KEY = 'AIzaSyC9gA0HENMSjweA4eRlFSVWYUuAXXFe2mI'
...
class GooMPy(object):
    ...
    #add functions to change coords
    def newCoords(self, dy, dx):
        #lonpix = _EARTHPIX + self.lon * math.radians(_pixrad)
        #sinlat = math.sin(math.radians(self.lat))
        #latpix = _EARTHPIX - _pixrad * math.log((1 + sinlat)/(1 - sinlat)) / 2
        
        self.lat = self.lat - dy/100
        self.lon = self.lon - dx/100
        
   # def change_coords(self,newlat, newlon):
    #    self.lat = newlat
     #   self.lon = newlon
      #  self._fetch_and_update()
